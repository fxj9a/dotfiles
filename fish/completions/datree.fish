...
function __datree_perform_completion
    __datree_debug "Starting __datree_perform_completion"
    ...
    set -l requestComp "DATREE_ACTIVE_HELP=0 $args[1] __complete $args[2..-1] $lastArg"
...
# - Obtain the completions and store them in the global __datree_comp_results
...
function __datree_prepare_completions
    ...
    set -l results (__datree_perform_completion)
    ...
    if test $nospace -ne 0; or test $nofiles -eq 0
        ...
        __datree_debug "prefix: $prefix"

        set -l completions (string match -r -- "^$prefix.*" $__datree_comp_results)
        set --global __datree_comp_results $completions
        __datree_debug "Filtered completions are: $__datree_comp_results"

        # Important not to quote the variable for count to work
        set -l numComps (count $__datree_comp_results)
        ...
        if test $numComps -eq 1; and test $nospace -ne 0
            ...
            set -l split (string split --max 1 \t $__datree_comp_results[1])
            ...
            if not string match -r -q "[@=/:.,]" -- "$lastChar"
                ...
                set --global __datree_comp_results $split[1] $split[1].
                __datree_debug "Completions are now: $__datree_comp_results"
            end
        end
...
# The call to __datree_prepare_completions will setup __datree_comp_results
